...
class SocNavEnv_v1(gym.Env):
    ...
    def compute_reward_and_ticks(self, action):
        ...
        if self.MAP_X/2 < self.robot.x or self.robot.x < -self.MAP_X/2 or self.MAP_Y/2 < self.robot.y or self.robot.y < -self.MAP_Y/2:
            self.robot_is_done = True
            reward = self.OUTOFMAP_REWARD - self.discomfortreward #self.alive  #self.balreward - self.disreward 
            # discomfort = round(reward,0)

            info["OUT_OF_MAP"] = True

        elif distance_to_goal < self.GOAL_THRESHOLD:
            self.robot_is_done = True
            reward = self.REACH_REWARD   + self.alive #+ self.discomfortreward
            info["REACHED_GOAL"] = True

        elif collision is True:
            self.robot_is_done = True
            reward = self.COLLISION_REWARD  - self.discomfortreward #self.alive 

            info["HUMAN_COLLISION"] = True
            info["COLLISION"] = True

        elif self.ticks > self.EPISODE_LENGTH:
            self.robot_is_done = True
            reward = self.MAX_STEPS_REWARD - self.discomfortreward #self.alive 
            # discomfort = round(reward,4)

            info["MAX_STEPS"] = True
        else:
            self.robot_is_done = False
            # self.alive = self.alive + self.ALIVE_REWARD #+ self.disreward
            # print("alive reward",self.alive)
            # reward = 0


            if self.USE_SNGNN > 0.0:
                # print('USE_SNGNN')
                ...
                if self.USE_SNGNN:
                    with torch.no_grad():
                        ...
                        # print('sngnn_value',sngnn_reward)
                    ...
                    self.discomfort = sngnn_reward #+ self.ALIVE_REWARD
                ...
                if dmin < self.DISCOMFORT_DISTANCE:
                    ...
                    discomfort = ((dmin - self.DISCOMFORT_DISTANCE) * self.DISCOMFORT_PENALTY_FACTOR) #* self.TIMESTEP + self.ALIVE_REWARD
                    # discomfort = round(discomfort,4)
                    self.discomfort = discomfort

                    # self.discomfortreward = self.discomfortreward + discomfort
                    # print("discomfort",discomfort)
                    # print("discomfortreward",self.discomfortreward)
            ...
            # self.alive = float(self.alive + self.ALIVE_REWARD + self.discomfort) #+ self.disreward
            self.alive = self.alive + self.ALIVE_REWARD #+ self.discomfort #+ self.disreward
            self.discomfortreward = self.discomfortreward + self.discomfort #+ self.disreward
            # self.alive = round(self.alive,4)
            # reward = float(self.ALIVE_REWARD + self.discomfort)
            reward = self.discomfort

...
class RNN_MODEL():
    def __init__(self, config:str, **kwargs) -> None:
        ...
        print(self.seq_len) 
        self.ckpt_dir = os.path.join(self.ckpt_dir, self.rnnsave + self.window + "_6_STEPAHEAD")
    ...
    def configure(self, config:str):
        ...
        if self.run_name is not None:
            self.writer = SummaryWriter('RNN_model_runs/'+RNN_runs  + self.window +"_6_STEPAHEAD")
        else:
    ...
    def train(self):
        ...
        for idx in range(1, self.max_step + 1):        # while self.global_step < self.max_step:
            ...
            if self.global_step % 1 == 0:
                ...
                torch.save(
                    d, os.path.join(self.ckpt_dir, '{:03d}six.pth.tar'.format(self.global_step//self.save_interval )))
